...
def compute_compressed_pub_key(private_key: bytes) -> bytes:
    ...
    compressed_pub_key = int.to_bytes(prefix, 1, BYTE_ORDER) + int.to_bytes(
        x, 32, BYTE_ORDER
    )
    return compressed_pub_key


def compute_child_keys(
...
def compute_binary_seed(bip39_phrase: str) -> bytes:
    binary_seed = hashlib.pbkdf2_hmac(
        HMAC_DIGEST_ALGO, bip39_phrase.encode(), b"mnemonic", 2048
    )
    return binary_seed
...
def main(bip39_phrase: str):
    ...
    # Level 0 Purpose
    level0_private_key, level0_chain_code = compute_child_keys(
        master_private_key, master_chain_code, 84, True
    )

    # Level 1 Coin Type
    level1_private_key, level1_chain_code = compute_child_keys(
        level0_private_key, level0_chain_code, COIN_TYPE, True
    )

    # Level 2 Account
    level2_private_key, level2_chain_code = compute_child_keys(
        level1_private_key, level1_chain_code, 0, True
    )

    # Level 3 Change
    level3_private_key, level3_chain_code = compute_child_keys(
        level2_private_key, level2_chain_code, 0, False
    )

    # Level 4 Index
    level4_private_key_1, level4_chain_code_1 = compute_child_keys(
        level3_private_key, level3_chain_code, 0, False
    )
    level4_private_key_2, level4_chain_code_2 = compute_child_keys(
        level3_private_key, level3_chain_code, 1, False
    )
    level4_private_key_3, level4_chain_code_3 = compute_child_keys(
        level3_private_key, level3_chain_code, 2, False
    )
    ...
    seg_wit_native_addr3 = compute_seg_wit_native_bitcoin_address(level4_private_key_3)
    print("-" * 15)

    print(f"Bitcoin address 1: {seg_wit_native_addr1}")
    print(
        f"Public key address 1: {compute_compressed_pub_key(level4_private_key_1).hex()}"
    )
    print(f"Private key address 1: {level4_private_key_1.hex()}")
    print("-" * 15)
    print(f"Bitcoin address 2: {seg_wit_native_addr2}")
    print(
        f"Public key address 2: {compute_compressed_pub_key(level4_private_key_2).hex()}"
    )
    print(f"Private key address 2: {level4_private_key_2.hex()}")
    print("-" * 15)

    print(f"Bitcoin address 3: {seg_wit_native_addr3}")
    print(
        f"Public key address 3: {compute_compressed_pub_key(level4_private_key_3).hex()}"
    )
    print(f"Private key address 3: {level4_private_key_3.hex()}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Bitcoin key generator", description="A generator of bitcoin key"
    )
